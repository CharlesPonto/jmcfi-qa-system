...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  return (
    <>
      ...
      {/* SIDEBAR - Background color #6A003A */}
      <aside className={`fixed top-0 left-0 w-64 h-screen bg-[#6A003A] text-white flex flex-col z-[70] shadow-2xl transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        ...
        {/* LOGO SECTION - VERTICALLY CENTERED TEXT */}
        <div className="relative px-7 pt-12 pb-10 flex-shrink-0">
          <div className="flex items-center gap-3 h-12"> {/* Fixed height to match logo for center logic */}
            {/* Logo Container */}
            ...
            {/* Text Container - Flex column centered vertically */}
        ...
        <nav className="relative flex-1 px-4 space-y-8 overflow-y-auto no-scrollbar pt-2 font-sans">
          ...
          <div className="space-y-1">
            <p className="px-4 text-[10px] font-bold text-white/20 uppercase tracking-[0.25em] mb-3">
              Core Menu
            </p>
    ...
    >
      {({ isActive }) => (
        <>
          ...
          <span className={`text-sm font-semibold tracking-tight transition-colors ${isActive ? "text-[#6A003A]" : "group-hover:text-white"}`}>
            {label}
          </span>
          {isActive && (
            <div className="ml-auto w-1.5 h-1.5 bg-[#6A003A] rounded-full" />
          )}
        </>
      )}
    </NavLink>
...
const Icon = ({ type, isActive }) => {
  const iconBase = "w-5 h-5 transition-colors flex-shrink-0";
